...
class TestGetaround(unittest.TestCase):
    ...
    def test_rental_options(self):
        rental = RentalRequest(rentals[0])
        rental.options = ['gps', 'baby_seat']
        price = Getaround.get_price(rental, cars[0])
        transactions = Getaround.get_transactions(price, rental.duration)
        Getaround.apply_rental_options(transactions, rental)

        self.assertEqual(transactions['driver'], -3700)
        self.assertEqual(transactions['owner'], 2800)
        self.assertEqual(transactions['insurance'], 450)
        self.assertEqual(transactions['assistance'], 100)
        self.assertEqual(transactions['drivy'], 350)

    def test_all_rental_options(self):
        rental = RentalRequest(rentals[0])
        rental.options = ['gps', 'baby_seat', 'additional_insurance']
        price = Getaround.get_price(rental, cars[0])
        transactions = Getaround.get_transactions(price, rental.duration)
        Getaround.apply_rental_options(transactions, rental)

        self.assertEqual(transactions['driver'], -4700)
        self.assertEqual(transactions['owner'], 2800)
        self.assertEqual(transactions['insurance'], 450)
        self.assertEqual(transactions['assistance'], 100)
        self.assertEqual(transactions['drivy'], 1350)
